...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    val meansGrouped = points.groupBy(findClosest(_,means))

    // Inizializate all means, because of some of them could have not any associated point.
    // And then, update with the grouped means
    means.map{(_,Seq())}.toMap ++ meansGrouped
  }
  ...
  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    ...
    // Inizializate all means, because of some of them could have not any associated point.
    // And then, update with the grouped means
    means.map{(_,ParSeq())}.toMap ++ meansGrouped
  }
  ...
  val convergedPoints = {(tuplePoints:(Point,Point), eta: Double) => tuplePoints._1.squareDistance(tuplePoints._2) <= eta}

  def converged(eta: Double, oldMeans: Seq[Point], newMeans: Seq[Point]): Boolean = {
    require(oldMeans.length == newMeans.length, "Old and new means must have the same length")
    (0 until oldMeans.length)
      .map(i => (oldMeans(i),newMeans(i)))
      .foldLeft(true){
        (acc:Boolean, elem:(Point,Point)) => acc && convergedPoints(elem,eta)
      }
  }
