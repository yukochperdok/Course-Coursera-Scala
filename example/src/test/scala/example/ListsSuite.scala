 ...
 */
 @RunWith(classOf[JUnitRunner])
   ...
   */
  test("one plus one is two")(assert(Lists.sum(List(1,1)) == 2))
  ...
  test("one plus one isn't three") {
    assert(Lists.sum(List(1,1)) != 3) // This assertion fails! Go ahead and fix it.
  }
  ...
  test("details why one plus one is not three") {
    assert(Lists.sum(List(1,1)) != 3) // Fix me, please!
  }
  ...
  test("max of a unique number") {
    assert(max(List(7)) === 7)
  }

  test("max fail with empty list") {
