...
package object barneshut {
  ...
  sealed abstract class Quad extends QuadInterface {
    ...
    def insert(bodies: coll.Seq[Body]): Quad = {
      bodies match {
        case Nil => this
        case xs: coll.Seq[_] => insert(xs.head).insert(xs.tail)
      }
    }
  }
  ...
  extends Quad {
    ...
    def calculateMass(bodies: coll.Seq[Body]): (Float, Float, Float) = {
      val mass = bodies.map(_.mass).sum
      val massX = bodies.map(q => q.mass * q.x).sum / mass
      val massY = bodies.map(q => q.mass * q.y).sum / mass
      (mass, massX, massY)
    }
    val (mass, massX, massY) = calculateMass(bodies)
    ...
    def insert(b: Body): Quad =
      if (size <= minimumSize) Leaf(centerX, centerY, size, bodies :+ b )
        ...
        Fork(nw, ne, sw, se).insert(bodies :+ b)
      }
  }
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case Leaf(_, _, _, bodies) =>
          ...
          for(i <- bodies) yield addForce(i.mass, i.x, i.y)
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface {
    ...
    def +=(b: Body): SectorMatrix = {
      val posX: Int =
        if (b.x <= boundaries.minX + sectorSize) 0
        else if (b.x >= boundaries.maxX - sectorSize) sectorPrecision - 1
        else ((b.x - boundaries.minX) / sectorSize).toInt

      val posY: Int =
        if (b.y <= boundaries.minY + sectorSize) 0
        else if (b.y >= boundaries.maxY - sectorSize) sectorPrecision - 1
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      ...
      for{
        i <- matrix.indices.par
      } yield matrix(i) = matrix(i).combine(that.matrix(i))
      this
    }
